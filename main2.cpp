...
class ObjPool {
    ...
    public:
        explicit ObjPtrWrapper(obj_ptr_t&& obj_ptr, ObjPool& parrent)
            : _obj_ptr { std::forward<obj_ptr_t>(obj_ptr) }
            , _parrent { parrent }
        {}
        ...
        ~ObjPtrWrapper() {
            _parrent.release(std::move(_obj_ptr));
        }
...
public:
    ...
    explicit ObjPool(size_t n, ObjArgs&&... obj_args)
        : _size {n}
    {
        for (size_t i = 0; i < _size; ++i) {
            _objects.emplace_back(
                new ObjT (std::forward<ObjArgs>(obj_args)...)
            );
        }
    }
...
uint64_t paperLeftSum(ObjPool<Printer>& obj_pool) {
    return std::accumulate(obj_pool._objects.cbegin(), obj_pool._objects.cend(), uint64_t{}, [](uint64_t sum, const auto& rhs) {
        return sum + rhs->paper();
    });
}
...
int main() {
    ...
    assert(paper_to_print_sm == printers * _paper_in_each - paperLeftSum(obj_pool)); // works only if papers at start >= papers to print

    std::cout << std::format("obj_pool expected size {}\nobj_pool real size {}\n", obj_pool.size(), obj_pool._objects.size());
